...
public class ImageInputStream extends Component {
	 ...
	 */
	public ImageInputStream(File file) throws InterruptedException, FileNotFoundException,
	...
	public ImageInputStream(String url, int i) throws InterruptedException, IOException,
		...
		if (url.endsWith(".gif") || url.endsWith(".jpg") || url.endsWith(".jpeg")) {
			...
			Image image = toolkit.getImage(ur);
	...
	/** Reads in the Image */
	public BufferedImage read() throws ImageNotSupportedException, IllegalPBMFormatException,
			IOException {
		...
		if (img == null) {
			...
			case PGM_RAW:
			case PGM_ASCII:
				im = new BufferedImage(X, Y, BufferedImage.TYPE_USHORT_GRAY);
				break;
			case PPM_ASCII:
			case PPM_RAW:
			...
			for (int y = 0; y < Y; y++) {
				for (int x = 0; x < X; x++) {
					...
					case PBM_ASCII:
						...
						color = new Color(value, value, value).getRGB();
						im.setRGB(x, y, color);
						break;
					case PGM_ASCII:
						g = readInt(data);
