...
public class Reduce implements ITransform {
    ...
    public BufferedImage apply(BufferedImage input) {
        ...
        float baseReduction = 1.5f;

        float xFactorFinal = this.xFactor;
        float yFactorFinal = this.yFactor;

        int xReductions = 0;
        int yReductions = 0;

        // While factor > 2, divide by 2
        while (xFactorFinal > baseReduction) {
            xFactorFinal /= baseReduction;
            ++xReductions;
        }
        while (yFactorFinal > baseReduction) {
            yFactorFinal /= baseReduction;
            ++yReductions;
        }

        BufferedImage output = ImageHelper.bufferedImageDeepCopy(input);
        while (xReductions > 0 || yReductions > 0) {
            output = Reduce.resample(output, baseReduction, baseReduction);
            --xReductions;
            --yReductions;
        }

        output = Reduce.resample(output, xFactorFinal, yFactorFinal);

        return output;
    }

    private static BufferedImage resample(BufferedImage input, float xFactor, float yFactor) {
        ...
        int outputWidth = (int) (inputWidth / xFactor);
        int outputHeight = (int) (inputHeight / yFactor);
        ...
        for (int x = 0; x < outputWidth; ++x) {
            for (int y = 0; y < outputHeight; ++y) {
                ...
                //System.out.println("Coordinates: (" + flatv + ", " + flatw + ")");
